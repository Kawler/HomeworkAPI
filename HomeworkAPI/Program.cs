...
builder.Services.AddScoped<ISubjectsRepository>(s =>
    new RawSqlSubjectsRepository(builder.Configuration.GetValue<string>("DefaultConnection")));
...
/*
const string connectionString = @"Data Source=DESKTOP-TJCTQAF;Initial Catalog=Homework;Pooling=true;Integrated Security=SSPI;TrustServerCertificate=True";
...
while (true)
{
    ...
    else if (command == "add-subject") 
    {
       ...
       do
       {
        ...
        Console.WriteLine("Введите новое название предмета: ");
        string newSubjectName = Console.ReadLine();
        Subjects newSubject = new(0,newClassroom, newSubjectName);
        subjectsRepository.Create(newSubject);
        Console.WriteLine("Данные обновленны");          
    }
